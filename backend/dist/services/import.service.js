...
class ImportService {
    ...
    leadRepo;
    failedRepo;
    constructor(jobRepo, leadRepo, failedRepo) {
     ...
     */
    async processJob(jobId, mappingSchema, filePath) {
        const job = await this.jobRepo.findById(jobId);
        if (!job) {
            logger_1.logger.error(`Job ${jobId} not found in repository. Aborting processing.`);
            return;
        }
        try {
            logger_1.logger.info(`Starting process for job: ${jobId} (${job.filename})`);
            ...
            await new Promise((resolve, reject) => {
                stream.on('data', async (row) => {
                    ...
                    const getValuesForField = (crmField) => {
                    ...
                    try {
                        ...
                        for (const re of rawEmails) {
                            allEmails.push(...(0, validation_1.extractEmails)(re));
                        }
                        // 2. Extract Phone Numbers
                        const rawPhones = getValuesForField('mobile');
                        const allPhones = [];
                        for (const rp of rawPhones) {
                            allPhones.push(...(0, validation_1.extractPhones)(rp));
                        }
                        ...
                        if (rawDate) {
                            const normDate = (0, date_1.normalizeDate)(rawDate, mappingSchema.date_format);
                            if (normDate) {
                                createdAt = normDate;
                            }
                            else {
                        ...
                        // Normalize Status
                        let status = 'GOOD_LEAD_FOLLOW_UP'; // Default fallback
                        ...
                        if (primaryPhone)
                            seenMobiles.add(primaryPhone);
                        // Validation error tracking
                        const errors = [];
                        if (primaryEmail && !(0, validation_1.isValidEmail)(primaryEmail)) {
                            errors.push(`Invalid email format: ${primaryEmail}`);
                        }
                        if (primaryPhone && !(0, validation_1.isValidPhone)(primaryPhone)) {
                        ...
                        logger_1.logger.error(`Error processing row ${rowNumber}`, err);
                        failedCount++;
                        failedBuffer.push({
                            row_number: rowNumber,
                            raw_data: row,
                ...
                stream.on('end', async () => {
                    try {
                        ...
                        logger_1.logger.info(`Completed job ${jobId}. Success: ${successCount}, Failed: ${failedCount}, Skipped: ${skippedCount}`);
                        resolve();
                    }
                    catch (err) {
                        reject(err);
                ...
                stream.on('error', (err) => {
                    logger_1.logger.error(`Streaming error during CSV process for job ${jobId}`, err);
                    reject(err);
                });
            });
        }
        catch (error) {
            logger_1.logger.error(`Failed to process CSV import job: ${jobId}`, error);
            await this.jobRepo.update(jobId, {
                status: 'FAILED',
